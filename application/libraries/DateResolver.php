...
class DateResolver
{
    ...
    public function resolve($text)
    {
        ...
        // ─── 0. Normalisasi alias Indonesia ───
        // Kontraksi: seminggu → 1 minggu, sebulan → 1 bulan, dll.
        $contractions = [
            'seminggu'  => '1 minggu',
            'sepekan'   => '1 minggu',
            'sebulan'   => '1 bulan',
        ...
        foreach ($angka_map as $kata => $digit) {
            // Hanya ganti jika diikuti unit waktu (hari/minggu/bulan/tahun)
            $text = preg_replace('/\b' . $kata . '\b\s*(?=hari|minggu|pekan|bulan|tahun)/', $digit . ' ', $text);
        }
