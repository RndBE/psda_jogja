...
<?php
$qstatus = $this->db->query('select waktu from ' . $this->session->userdata('tabel') . ' USE INDEX (waktu) where code_logger="' . $this->session->userdata('idlogger') . '" order by waktu desc limit 1');
			...
			<div class="col-12 col-md">
				...
				<div class="row g-3 align-items-center justify-content-end">
					...
					<div class="col-6 col-md-auto d-flex">
						<a class="d-none d-lg-block btn w-100 me-3" href="<?= base_url() ?>awlr/livedata">
							<!-- Download SVG icon from http://tabler-icons.io/i/settings -->
...
<div class="page-body">
								...
								<div class="h3 m-0 pt-2">
									<form action="<?= base_url() ?>awlr/set_pos" method="post" id="form-pos">
										...
										<select type="text" name="pilihpos" class="form-select" placeholder="Pilih Pos AWLR" onchange="validate_form()" id="select-pos" value=" ">
											...
											<option value="<?= $mnpos->idLogger ?>" <?= ($mnpos->idLogger == $this->session->userdata('idlogger')) ? 'selected' : '' ?>><?= str_replace('_', ' ', $mnpos->namaPos) ?></option>
											<?php endforeach ?>
										</select>
									</form>
								</div>
	...
	echo form_open('awlr/set_parameter'); ?>
									<select type="text" name="mnsensor" class="form-select" placeholder="Pilih Parameter" onchange="this.form.submit()" id="select-parameter" value=" ">
										...
										<option value="<?= $mnparameter->idParameter ?>" <?= ($mnparameter->idParameter == $this->session->userdata('idparameter')) ? 'selected' : '' ?>><?= str_replace('_', ' ', $mnparameter->namaParameter) ?></option>
										<?php endforeach ?>
									</select>
									<?php echo form_close() ?>
...
<div class="modal fade" id="tes" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-sm">
		<div class="modal-content">
			...
			<div class="modal-body text-center" id="tw">
				<?php
	$data20 = $this->db->query("select * from awlr USE INDEX (waktu) where code_logger = '" . $this->session->userdata('idlogger') . "' AND waktu >='" . $this->session->userdata('pada') . " 00:00'  AND waktu <= '" . $this->session->userdata('pada') . " 23:59'")->result_array();
	$current_time = time();
	...
	if ($this->session->userdata('pada') == date('Y-m-d')) {
		...
		$res = number_format((count($data20) / $total_minutes * 100), 2);
		$res2 = $res . ' %';
	} else {
		$tgl = $this->session->userdata('pada');
		$total_minutes = 1440;
		$res = number_format((count($data20) / 1440 * 100), 2);
		$res2 = $res . ' %';
	}
				...
				<h2><?= count($data20) ?> / <?= $total_minutes ?></h2>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog modal-lg modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="result"></div>
				<form action="<?= base_url('awlr/do_upload') ?>" class="dropzone" id="my-dropzone"></form>
			</div>
...
<script>
	...
	$("#process-btn").on("click", function () {
		...
		$.ajax({
			url: "<?= base_url('awlr/process_files') ?>",
			method: "POST",
