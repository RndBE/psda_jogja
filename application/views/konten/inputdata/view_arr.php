<?php
 $query_inf=$this->db->query('select serial_number from t_informasi where logger_id = "'.$this->session->userdata('log_id').'"');
...
<body>
<h1>Data Input ARR</h1> <?php echo 'ID LOGGER '.$this->session->userdata('log_id').' ##### |'.$this->session->userdata('tgl_arr').'| Serial Number :'.$a; ?>

<hr/>



<?php echo form_open('datamasuk/sesi_loggerarr');?>
Id Logger <input type="text" name="logger_id" />
<input name="btnlog" value="Cari" type="submit"/>
<?php echo form_close();?>
	<?php 
	echo form_open('datamasuk/tgl_arr');?>
	<label for="tgl">Tanggal:</label>
	<input type="date" id="tgl" name="tgl">
	<input name="btntgl" value="Cari" type="submit"/>
	<?php echo form_close();?>
	
<?php echo form_open('datamasuk/data_arr');?>
...
<?php      
   foreach($data_arr->result() as $row){
      ?>
     <tr>
         ...
         <td>&nbsp;<?php echo anchor('datamasuk/edit_arr/'.$row->id,'Sunting '); ?>&nbsp;</td>
         <td>&nbsp;<?php echo anchor('datamasuk/hapus_arr/'.$row->id,' Hapus'); ?>&nbsp;</td>
