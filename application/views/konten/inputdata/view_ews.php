<html>
<head>
   <title>Data EWS</title>
</head>
<body>
<h1>Data Input EWS</h1> <?php echo 'ID LOGGER '.$this->session->userdata('log_id').' ##### |'.$this->session->userdata('tgl_ews').'|'; ?>

<hr/>



<?php echo form_open('datamasuk/sesi_loggerews');?>
Id Logger <input type="text" name="logger_id" />
<input name="btnlog" value="Cari" type="submit"/>
<?php echo form_close();?>
	<?php 
	echo form_open('datamasuk/tgl_ews');?>
	<label for="tgl">Tanggal:</label>
	<input type="date" id="tgl" name="tgl">
	<input name="btntgl" value="Cari" type="submit"/>

	<?php //echo form_close();?>
<?php echo form_open('datamasuk/data_ews');?>
...
<?php      
   foreach($data_ews->result() as $row){
      ?>
     <tr>
